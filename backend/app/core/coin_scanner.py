...
(millions of coins). Uses a fast scoring pipeline that combines:
  ...
  2. Technical score (trend, momentum, volume, volatility)
  3. Smart Money score (sweep+hold+OB confluence)
  4. Risk-adjusted return (Sharpe-like, expected profit / risk)
  5. Capital flow + on-chain activity
...
def scan_coins(candidates: List[CoinCandidate], top_n: int = 20,
    ...
    ProfitRank = 0.35 * TokenScore + 0.25 * TechScore +
                 0.20 * RiskAdjReturn_normalized + 0.20 * TimingQuality
    ...
    # Step 2: normalize risk_adj_return to 0-100
